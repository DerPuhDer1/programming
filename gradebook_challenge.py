...
for row in range(len(gradebook)):
    total = 0
    for columns in range(len(gradebook[row])):
        total = total + gradebook[row][columns]
    print(f"Student {row + 1}'s average score is: {total / (columns + 1):.2f}")

print("\nAssignment Averages:")
#iterate 15 x 10
#Iterate over each assignment: 15
for assignment in range(len(gradebook[0])):
    #iterate over each student: 10
    total = 0
    for student in range(len(gradebook)):
        #Calculate the total score for each assignment
